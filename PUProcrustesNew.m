function [Q,D,Sfcorr2,OSflag,Nfft,e] = PUProcrustesNew(B,A,Nfftmax,Delta,NOrd);
%[Q,D,S,OSFlag] = PUProcrustesNew(B,A,Nfft,Delta,N);
% 
...
%      if k ==2, save; end;
   ...
   if OSflag==1,
      ...
      for m = 1:size(Sfcorr2,1),                % iterate for singular values
         disp(sprintf('checking %d-th singular value',m))
         % find sign changes
         ...
         if length(ZerosIndex)>0,               % singular value has zero crossings
            if QuietMode==false,
...
%            if QuietMode==false, plot(Omegas/2/pi,zeros(size(Omegas)),'ko'); drawnow; end;
            ...
            if mod(length(ZerosIndex),2)~=0,
               [b,a] = SingularValueAllpassSwitchCompact([Omegas Omegas+2*pi]/2,2*AllpassOrd);
               b = b(1:2:end); a = a(1:2:end);
%               disp('designed allpass for an odd number of zeros');  
%               save design.mat b a Omegas
%               return;
            else               
               [b,a] = SingularValueAllpassSwitchCompact(Omegas,AllpassOrd);  
%               disp('designed allpass for an even number of zeros');  
            end;
...
%         Af(m,:) = fft(circshift(At(m,:),-Delays(m),2),Nfft,2);  
         Af(m,:) = fft(At(m,:),Nfft,2) .* exp(1i*(0:(Nfft-1))/Nfft*2*pi*Delays(m));  
%         if mod(length(ZerosIndex),2)~=0,
%            Af(m,:) = Af(m,:).*exp(-(0:(Nfft-1))/Nfft*pi);
%         end;   
      end;            
      % MaxDelay = max(Delays);   
      D = D.*Af(:,NonZeroBins2);
